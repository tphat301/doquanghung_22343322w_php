...
$requick = array(
	/* Tin tức */
	array("tbl" => "news_list", "field" => "idl", "source" => "news", "com" => "ep-coc-be-tong", "type" => "ep-coc-be-tong"),
	array("tbl" => "news_cat", "field" => "idc", "source" => "news", "com" => "ep-coc-be-tong", "type" => "ep-coc-be-tong"),
	array("tbl" => "news", "field" => "id", "source" => "news", "com" => "ep-coc-be-tong", "type" => "ep-coc-be-tong", "menu" => true),
...
switch ($com) {
	...
	case 've-chung-toi':
		...
		$titleMain = vechungtoi;
		break;

	case 'ep-coc-be-tong':
		$source = "news";
		$template = isset($_GET['id']) ? "news/news_detail" : "news/epcocbetong";
		$seo->set('type', isset($_GET['id']) ? "article" : "object");
		$type = $com;
		$titleMain = "Ép cọc bê tông";
		break;
