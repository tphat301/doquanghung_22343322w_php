		...
		<?php for($i=0;$i<$numberPhoto;$i++) { $numb = $i+1; ?>
			<div class="card card-primary card-outline text-sm">
	            ...
	            <div class="card-body">
					...
					<?php if(isset($config['photo']['man_photo'][$type]['quantity_photo']) && $config['photo']['man_photo'][$type]['quantity_photo'] == true) { ?>
						<div class="row">
							<div class="form-group col-md-4">
								<label class="d-block" for="quantity">Số lượng:</label>
								<div class="input-group">
									<input type="number" class="form-control quantity text-sm" name="dataMulti[<?=$i?>][quantity]" id="quantity<?=$i?>" placeholder="Số lượng" value="<?=(!empty($flash->has('quantity'.$i))) ? $flash->get('quantity'.$i) : @$item['quantity'.$i]?>">
